...
const app = express();
const port = 3000;
const JWT_SECRET = "cafe-management-secret-key";
...
// Get menu items (from table.js - no authentication required)
...
app.put("/api/menu/:id", authenticateToken, (req, res) => {
  const { name, price, category, description, image_url, availability } =
    req.body;
...
// Get tables (from table.js - no authentication required)
...
// Table Reservation - from table.js
      ...
      db.query(
        ...
        (err, results) => {
          ...
          if (results.length > 0) {
            return res
              .status(400)
              .json({ error: "Table is already reserved for this time" });
          }
...
app.post("/api/orders", authenticateToken, (req, res) => {
  ...
  const itemPromises = items.map((item) => {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT id, price, availability FROM menu_items WHERE id = ?",
        [item.id],
        (err, results) => {
          if (err) {
            reject(err);
      ...
      db.beginTransaction((err) => {
        ...
        db.query(
          ...
          (err, results) => {
            ...
            const billValues = orderItems.map((item) => [
              orderId,
              item.id,
              item.quantity,
              item.price,
...
app.get("/api/dashboard", authenticateToken, (req, res) => {
  ...
  db.query(
    "SELECT COUNT(*) as pendingReservations FROM tables WHERE status = ?",
    ["reserved"],
    (err, results) => {
      if (err) {
